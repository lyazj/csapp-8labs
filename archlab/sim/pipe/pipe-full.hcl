...
word f_pc = [
	...
	### M_icode == IJXX && M_valA >= M_valE && !M_Cnd : M_valA;
	### M_icode == IJXX && M_ifun != 0 && M_valA < M_valE && M_Cnd : M_valE;
...
word f_predPC = [
	...
	### f_icode == IJXX && (f_ifun == 0 || f_valP >= f_valC) : f_valC;
	f_icode == IJXX : f_valC;
	1 : f_valP;
];
...
word d_valA = [
	...
	D_icode == IJXX  : D_valP;    # Use incremented PC
...
bool D_bubble =
	# Mispredicted branch
	(E_icode == IJXX && !e_Cnd) || 
	### (E_icode == IJXX && E_valA >= E_valC && !e_Cnd) ||
	### (E_icode == IJXX && E_ifun != 0 && E_valA < E_valC && e_Cnd) ||
	# Stalling at fetch while ret passes through pipeline
